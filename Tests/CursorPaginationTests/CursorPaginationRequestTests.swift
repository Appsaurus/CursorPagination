...
class CursorPaginationRequestTests: CursorPaginationTestCase {
    ...
    override func addRoutes(to router: Routes) throws {
        ...
        router.get("models") { request async throws -> CursorPage<KitchenSink> in
            return try await KitchenSink.paginate(request: request,
                                             sorts: .descending(\.$dateField), .ascending(\.$stringField))
        }

        router.get("dynamicModels") { request async throws -> CursorPage<KitchenSink> in
            return try await KitchenSink.paginate(request: request)
        }
    }
    
    func testPaginationRequest() async throws{
        try await seedModels(40)
        let existingModels = try await KitchenSink.query(on: app.db).all()
        ...
        repeat{
            ...
            try await app.test(.GET, "models", queryItems: queryItems, afterResponse: { response in
    ...
    func testDynamicPaginationRequest() async throws{
        try await seedModels(40)
        let existingModels = try await KitchenSink.query(on: app.db).all()
        let expectedTotalCount = existingModels.count
        let limit: Int = 5
        ...
        repeat{
            ...
            try await app.test(.GET, "dynamicModels", queryItems: queryItems, afterResponse: { response in
                let page: CursorPage<KitchenSink> = try response.content.decode(CursorPage<KitchenSink>.self)
...
public extension XCTApplicationTester {
    ...
    func test(_ method: NIOHTTP1.HTTPMethod,
                     ...
                     afterResponse: (XCTVapor.XCTHTTPResponse) throws -> () = { _ in }) async throws -> XCTVapor.XCTApplicationTester {
