...
extension QueryBuilder where Model: CursorPaginatable {
	...
	public func filter<E, R>(query: QueryBuilder<E, R>, for sort: QuerySort, startingAt cursor: CursorPart, inclusive: Bool = true) throws -> QueryBuilder<E, R>{
		...
		case (true, .descending):
			let idKey: String = try E.idKey.makeQueryField().name
			if key == idKey {
	...
	public func ensureUniqueSort<E: CursorPaginatable>(sorts: inout [QuerySort], modelType: E.Type) throws {
		if sorts.count == 0 { sorts.append(contentsOf: modelType.defaultPageSorts) }
		let idKey: String = try modelType.idKey.makeQueryField().name
...
fileprivate extension QueryBuilder{
	...
	@discardableResult
	fileprivate  func filter(_ field: String, _ type: QueryFilterType<Model.Database>, _ value: QueryFilterValue<Model.Database>) -> Self {
