...
extension QueryBuilder where Model: CursorPaginatable {
    ...
    public func paginate(request: Request,
                         sorts: [CursorSort<Model>] = []) async throws -> CursorPage<Model> {
        let params = try request.query.decode(CursorPaginationParameters.self)
        return try await paginate(cursor: params.cursor, limit: params.limit, sorts: sorts)
    }
    ...
    public func paginate(cursor: String?,
        ...
        return try await paginate(cursor: cursor, cursorBuilder: defaultCursorBuilder(sorts), limit: limit, sorts: sorts)
    }
    ...
    public func paginate(cursor: String?,
        ...
        let data = try await query.range(...limit).all()

        var resultData = data
        var nextPageCursor: String? = nil
        if resultData.count == limit + 1, let nextFirstItem: Model = resultData.last {
            ...
            resultData.remove(at: resultData.count - 1)
        }
        
        return CursorPage(
            nextPageCursor: nextPageCursor,
            data: resultData,
            remaining: total - resultData.count
        )
    }
...
extension QueryBuilder where Model: CursorPaginatable {
    ...
    public func paginate(request: Request) async throws -> CursorPage<Model> {
        let params = try request.query.decode(DynamicCursorPaginationParameters<Model>.self)
        debugPrint("Params \(params)")
        return try await paginate(cursor: params.cursor, limit: params.limit, sorts: try params.cursorSorts())
