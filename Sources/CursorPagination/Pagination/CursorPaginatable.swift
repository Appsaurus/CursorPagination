...
extension CursorPaginatable {
    ...
    public static func paginate(request: Request,
                                sorts: [CursorSort<Self>]) async throws -> CursorPage<Self> {
        return try await query(on: request.db).paginate(request: request, sorts: sorts)
    }

    public static func paginate(request: Request,
                                sorts: CursorSort<Self>...) async throws -> CursorPage<Self> {
        
        return try await paginate(request: request, sorts: sorts)
    }
    ...
    public static func paginate(on conn: Database,
                                ...
                                limit: Int? = nil,
                                sorts: [CursorSort<Self>]) async throws -> CursorPage<Self> {
        return try await query(on: conn).paginate(cursor: cursor, limit: limit, sorts: sorts)
    }
